...
export function RepsProgressionGraph({ workoutLogs, programs, exercises }: RepsProgressionGraphProps) {
          ...
          if (progressArray) {
            ...
            if (workoutIndex > 0) {
              ...
              for (let i = workoutIndex - 1; i >= 0; i--) {
                ...
                if (prevData && prevData.weight !== null) {
            ...
            if (chartItem && exercise) {
              ...
              if (dataPoints && dataPoints[workoutIndex]) {
                ...
                if (exerciseData.weight !== null) {
                  return `${exerciseName}: ${value} avg reps @ ${Math.round(exerciseData.weight * 10) / 10} ${weightUnit === 'kg' ? 'Kgs' : 'lbs'} avg`;
                }
              }
            }
            
            return `${context.dataset.label}: ${value} avg reps`;
          },
        },
      },
    },
  ...
  if (workoutCharts.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 bg-accent-light dark:bg-gray-700/30 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600">
        <div className="text-center">
          ...
          <p className="text-gray-500 dark:text-gray-500 text-sm mt-1">
            Log your workouts to see your reps progression over time
          </p>
        </div>
      </div>
    );
